...
class GraphViewProvider {
        ...
        this._panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.type) {
                case 'getKgData':
                    ...
                    break;
                case 'searchEntities':
                    const results = this._stateManager.searchEntities(message.query, message.entityType, message.limit || 50);
                    this._panel.webview.postMessage({
                        type: 'searchResults',
                        results: results
                    });
    ...
    show() {
        ...
        this._panel = vscode.window.createWebviewPanel('kgBuilder.graphView', 'Knowledge Graph', vscode.ViewColumn.One, {
...
<body>
     ...
     */
    getEntityPreviewHtml(kgData) {
        const entities = Object.values(kgData.entities).slice(0, 10);
        const typeClasses = {
            CLASS: 'type-class',
